      ...
      <nav className="sticky top-0 z-30 bg-surface border-b border-border backdrop-blur-sm bg-white/80">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <Link href="/dashboard" className="hana-brand">
              <span className="b-hana">Hana Power On</span>
              <span className="b-power">하나파워온스토어</span>
              <span className="b-sub">레벨업 컨설팅 · {isAdmin ? '관리자' : '디자이너'}</span>
            </Link>
            ...
            <div className="flex items-center gap-1">
              ...
              {/* 초안 — 디자이너=업로드/수정시안, 관리자=초안 체크/최종 발송 */}
          ...
          <div className="flex items-center gap-3">
            {role === 'admin' && (
              <Link href="/users" className="text-xs text-text-tertiary hover:text-text-secondary">사용자 관리</Link>
            )}
