...
export function buildSectionDesignPrompt(opts: SectionDesignPromptOptions): string {
  ...
  lines.push(
    ...
    `- ⚠️ 이미지 안의 모든 텍스트는 반드시 한국어(한글)로만 작성. 영어·로마자 단어 절대 금지(₩ 가격, 숫자 %는 허용).`,
    `- 위에 제공된 제목/부제목/본문 문구를 한 글자도 바꾸지 말고 한글 그대로 정확히 렌더링(철자 오류·오타 금지).`,
    `- 텍스트 가독성 최우선: 충분한 대비, 최소 18pt, Noto Sans KR 스타일`,
    `- 제공된 제품 사진과 스타일링 사진을 자연스럽게 배치`,
    `- 실제 ${plat?.name ?? '이커머스 플랫폼'}에 올라갈 상세페이지 디자인`,
    `- 전문 디자이너가 만든 것처럼 완성도 높은 디자인`,
    ``,
    `[금지사항]`,
    `- 영어/로마자 텍스트 (한국 이커머스 상세페이지이므로 모든 카피는 한국어)`,
    `- 철자 오류·오타가 있는 텍스트`,
    `- 와이어프레임/스케치/기획안 느낌`,
