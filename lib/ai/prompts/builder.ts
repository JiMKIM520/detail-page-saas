...
| 금지 표현 | 허용 대체 표현 |
|-----------|--------------|
${category.forbiddenExpressions.map((e) => `| ${e.forbidden} | ${e.allowed} |`).join('\n')}

### 필수 섹션 구성
${category.requiredSections.map((s, i) => `${i + 1}. ${s}`).join('\n')}

### 톤 & 보이스
${category.tone}

### 촬영 가이드
${category.shootingGuide}`
    : ''
...
### 이미지 스펙
- 권장 너비: ${platform.imageSpecs.width}px
${platform.imageSpecs.maxHeight ? `- 최대 높이: ${platform.imageSpecs.maxHeight}px` : ''}
- 최대 파일 크기: ${platform.imageSpecs.maxFileSize}
${platform.imageSpecs.thumbnailSize ? `- 대표이미지 크기: ${platform.imageSpecs.thumbnailSize}` : ''}
- 지원 포맷: ${platform.imageSpecs.formats}

### 타겟 고객
${platform.targetAudience}

### 레이아웃 규칙
${platform.layoutRules}

### 전환율 최적화 팁
${platform.conversionTips}

### 흔한 실수 (반드시 피할 것)
${platform.commonMistakes}`
    : ''

  const compat = category && platform
    ? getCompatibility(categorySlug, platformSlug)
    : null
  const compatLabel = compat ? getCompatibilityLabel(compat) : null
  const compatSection = compatLabel
    ? `\n## 카테고리×플랫폼 궁합\n${compatLabel.emoji} ${compatLabel.label}: ${compatLabel.description}`
    : ''

  const crossTip = getCrossStrategyTip(categorySlug, platformSlug)
  const crossSection = crossTip
    ? `\n## 핵심 조합 전략\n${crossTip}`
    : ''

  return `당신은 이커머스 상세페이지 기획 전문가입니다.
...
- 섹션 수는 최소 10개(이상적으로 12~14개), 각 섹션의 카피는 실제 상세페이지에 바로 사용 가능한 수준. 억지로 섹션을 늘리지 말 것.
...
위 텍스트 정보와 함께 첨부된 이미지를 분석하여 ${project.platform_name} 플랫폼에 최적화된 ${project.category_name} 카테고리 상세페이지 스크립트를 생성해 주세요.
