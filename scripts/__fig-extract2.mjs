...
const sectionIds = new Set(nodes.filter((n) => n.type === 'SECTION').map((n) => gid(n.guid)))
const parentOk = (n) => n.parentIndex && (canvasIds.has(gid(n.parentIndex.guid)) || sectionIds.has(gid(n.parentIndex.guid)))
const tops = nodes.filter((n) => (n.type === 'FRAME' || n.type === 'SYMBOL') && parentOk(n) && n.visible !== false)
fs.mkdirSync(process.argv[3] ?? 'frames', { recursive: true })
const index = []
tops.forEach((top, i) => {
  const out = []
  const budget = { max: Number(process.argv[4] ?? 450), truncated: false }
