...
import type { Script, ProjectBrief } from '../agents/types'
...
;(async () => {
  ...
  console.log('=== [6단계] QA 테스트 ===\n')

  const script = loadJson<Script>(`${dirs.script}/script.json`)

  const result = await runQA(script, brief, dirs.base)
  ...
  if (report.issues.length > 0) {
    ...
    report.issues.forEach((issue, i) => {
      ...
      if (issue.suggestion) console.log(`     제안: ${issue.suggestion}`)
    })
  } else {
    console.log('  이슈 없음')
  }

  console.log(`\n✅ 저장 완료: ${dirs.base}/compliance-report.json`)
