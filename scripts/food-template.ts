/**
 * 식품 카테고리 슬롯형 상세페이지 템플릿 (파라메트릭).
 * 두브로 그래놀라급 7섹션 구조 + 공유 디자인 토큰(브랜드컬러 변수화).
 * renderFoodDetail(data) → 완성 HTML 문자열. 에이전트가 data(슬롯)만 채우면 됨.
 *
 * 섹션: HERO · INTRO · INGREDIENT_DIAGRAM · CHECK_POINT · HOW_TO_EAT · WHEN_TO_EAT · TRUST · LINEUP · CTA
 */
...
export function renderFoodDetail(d: FoodDetailData): string {
  ...
  return head(t, `${d.brand} ${d.product}`) +
    hero + intro + ing + cp + how + when + spec + trust + lineup + cta +
    `</body></html>`
}
