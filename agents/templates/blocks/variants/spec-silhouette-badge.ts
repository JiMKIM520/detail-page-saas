...
export const specSilhouetteBadge = defineBlock<Data>({
  ...
  imageSlots: 1,
  describe:
    '용량·치수 인포그래픽. 라틴 대제목(size info) + 한국어 섹션 타이틀 + 용량 카피 → 제품 실루엣(이미지 또는 CSS 폴백) 위에 가로 치수선 3개와 브랜드 컬러 원형 용량 배지를 우상단 오버랩. 하단 치수 범례 행 + 오차 안내. 텀블러·보틀·통조림 등 용량·사이즈 강조 식품·생활용품에 적합. 주의: SVG 폴백이 보틀(병) 실루엣이므로 보틀형이 아닌 제품은 반드시 productImage(누끼)를 채울 것.',
  schema,
  css: `
/* ── 최상위 래퍼 ── */
