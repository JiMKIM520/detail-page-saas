...
/* ── 방사형 스테이지 ── */
.cixn-stage{position:relative;width:100%;max-width:540px;margin:0 auto;aspect-ratio:1/.72}
/* 제품 이미지 / 빈 원 중심 */
.cixn-hub{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
  width:42%;aspect-ratio:1/0.68;border-radius:var(--shape-photo, calc(var(--r-scale,1)*16px));
...
`,
  render: (d, { esc, richSafe, icon }) => {
    ...
    const startDeg = -90
    const rx = 42 // % of stage width  (수평 반경)
    const ry = 44 // % of stage height (수직 반경)
    const cx = 50
    const cy = 50
