...
export const storyPair = defineBlock<Data>({
  ...
  schema,
  css: `
.sy{position:relative;padding:70px var(--pad-x,56px) 60px;background:var(--bg);text-align:center}
.sy-h{margin-top:18px;font-size:38px}
.sy-pair{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin:38px 0 0}
.sy-media{width:100%;height:240px;object-fit:cover;border-radius:var(--shape-photo, calc(var(--r-scale,1)*20px));box-shadow:0 16px 32px -18px rgba(42,33,24,.4)}
