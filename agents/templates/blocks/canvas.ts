...
export const ARCHETYPE_CANVAS: Record<string, ArchetypeCanvas> = {
  hero: {
    form: '제품 성격에 맞는 아형 하나를 heroStyle 필드로 명시 — points(카피+체크포인트 2~4: 기능·신뢰 소구) / mood(감성 카피 1~2줄+무드 비주얼, 포인트 없음: 감성·프리미엄) / badge(수상·인증 뱃지+한 줄 훅: 권위·친근 소구). 대표 이미지 1. 헤드라인은 수치·스펙 나열 금지 — 소비자의 변화·감각·욕망을 담은 한 문장(수치는 서브카피·포인트 행으로)',
  },
...
히어로 헤드라인 규칙: 수치·스펙 나열("20g / 105kcal"류) 금지 — 소비자가 얻는 변화·감각·욕망을
한 문장으로. 수치는 서브카피나 포인트 행에 배치한다.
카피 반복 규칙: 동일 수치 클레임은 문서 전체에서 최대 2회 — 3회째부터는 비교 기준·권장량 환산·
사용 시나리오 등 새 정보를 더한 변형으로 쓴다.`
}
