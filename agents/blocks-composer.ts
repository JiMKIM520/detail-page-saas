...
- VISUAL RHYTHM (CRITICAL for premium depth — a flat single-tone page reads as low quality): when a PAGE BLUEPRINT is given, its scene tone design is BINDING — follow it, do not re-count dark blocks yourself. Only when composing WITHOUT a blueprint: count in SCENES (not blocks) — 1~3 dark scenes per page, never 3 consecutive scenes of the same tone, brand-story and closing lean dark/dramatic. Pick DIFFERENT dark variants each page.
